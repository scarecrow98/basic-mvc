...
class QueryBuilder {
    ...
    public function where($statement) {
        if (empty($statement) || !is_string($statement)) {
            throw new Exception("Function where expects its parameter to be string");
        }
    ...
    public function map($class) {
        if (!class_exists($class)) {
            throw new Exception("Function map cannot find the passed class");
        }

        $this->mapped_class = $class;
